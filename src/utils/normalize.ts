export const decodeHtmlEntities = (text: string): string => {
  if (!text) return text;
  try {
    // Minimal entity decode for common cases (no DOM on RN)
    return text
      .replace(/&amp;/g, '&')
      .replace(/&lt;/g, '<')
      .replace(/&gt;/g, '>')
...
export const getPlayableUrl = (song: any): string => {
  if (!song) return '';
  // downloadUrl can be an array of objects or strings
  try {
    const d = song.downloadUrl || song.download_url || song.downloads;
    if (Array.isArray(d) && d.length > 0) {
      // if array of strings
      if (typeof d[0] === 'string') return d[0];
      // array of objects - try pick by quality or bitrate
      const copy = [...d];
      copy.sort((a: any, b: any) => {
        const qa = parseInt((a?.quality || a?.bitrate || '').toString().replace(/[^0-9]/g, '')) || 0;
        const qb = parseInt((b?.quality || b?.bitrate || '').toString().replace(/[^0-9]/g, '')) || 0;
        return qb - qa;
      });
      const first = copy[0];
      if (first) return first.url || first.link || first[Object.keys(first)[0]] || '';
    }

    // if downloadUrl is a single string or media fields
    if (typeof d === 'string') return d;

    return song.media_url || song.media_preview_url || song.perma_url || song.url || '';
  } catch (e) {
    return song.media_url || song.media_preview_url || song.perma_url || song.url || '';
  }
};
